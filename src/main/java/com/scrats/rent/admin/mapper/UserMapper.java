...
public interface UserMapper extends BaseMapper<User> {
    ...
    @Select("<script>select u.* " +
            ...
            "<if test='user.deleteTs != null and user.deleteTs > 0'>and u.delete_ts > 0</if>" +
            "<if test='user.deleteTs == null or user.deleteTs == 0'>and u.delete_ts = 0</if>" +
            "<if test='user.checkTs != null and user.checkTs == 0'>and u.check_ts = 0</if>" +
            "<if test='user.checkTs != null and user.checkTs > 0'>and u.check_ts > 0</if>" +
            "</script>")
    ...
    @Select("<script>select u.* " +
            ...
            "where 1=1 and ur.role_code = #{roleCode}" +
            "<if test='user.userId != null'>and u.user_id = #{user.userId}</if>" +
            "<if test='user.name != null and user.name !=\"\"'>and u.name like concat('%', #{user.name}, '%')</if>" +
            "<if test='user.phone != null and user.phone !=\"\"'>and u.phone like concat('%', #{user.phone}, '%')</if>" +
            "<if test='user.qq != null and user.qq !=\"\"'>and u.qq like concat('%', #{user.qq}, '%')</if>" +
            ...
            "<if test='user.checkTs != null and user.checkTs > 0'>and u.check_ts > 0</if>" +
            "</script>")
    List<User> getListWithRoleByUser(@Param("user") User user, @Param("roleCode") String roleCode);
}
